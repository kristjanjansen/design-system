...
test("exports forwardRef component", () => {
  expect(TextField.$$typeof?.toString()).toBe("Symbol(react.forward_ref)");
});
...
test("calls onChange with value", () => {
  ...
  const container = render(<TextField onChange={handleChange} />);
  const input = container.querySelector("input")!;
  act(() => {
...
test("supports disabled and required", () => {
  const container = render(<TextField disabled required />);
  const input = container.querySelector("input")!;
...
test("passes through native HTML attributes", () => {
  const container = render(<TextField name="email" placeholder="Enter email" aria-label="Email" />);
  const input = container.querySelector("input")!;
